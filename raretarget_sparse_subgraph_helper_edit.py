...
class GraphHelper:
    ...
    def __init__(self, nodes_map: Dict[str, any], edges_map: Dict[str, any], device: torch.device):
        ...
        self.predicate_indices_map = {predicate: idx for idx, predicate in enumerate(self.predicates_list)}

        self.nodes_list = list(self.nodes_map.keys())
        self.num_nodes = len(self.nodes_list)
        self.node_indices_map = {node_key: idx for idx, node_key in enumerate(self.nodes_list)}

        # Build both the dense tensor and adjacency lists
        self.adjacency_tensor, self.adjacency_list = self.create_adjacency_maps(self.edges_map, device)

        # Build reverse-adjacency list for efficient back-BFS
        self.reverse_adjacency_list = defaultdict(set)
        for src, nbrs in self.adjacency_list.items():
            for dst in nbrs:
                self.reverse_adjacency_list[dst].add(src)

        # Also keep each predicate’s sparse adj matrix
        self.A_r_sparse = [
            self.adjacency_tensor[r].to_sparse()
            for r in range(self.num_predicates)
        ]
    ...
    def create_adjacency_maps(self, edges_map: dict, device: torch.device) -> Tuple[torch.Tensor, Dict[str, Set[str]]]:
        """Create both:
           - predicate_adj_tensor: shape [P, V, V] dense float tensor
           - adjacency_list: Dict[src_node_id, Set[dst_node_id]]
        """
        adjacency_list = defaultdict(set)
        predicate_adj_tensor = torch.zeros(
            self.num_predicates, self.num_nodes, self.num_nodes, device=device
        )

        for edge in edges_map.values():
            p = edge["predicate"]
            pi = self.predicate_indices_map[p]
            si = self.node_indices_map[edge["subject"]]
            oi = self.node_indices_map[edge["object"]]
            predicate_adj_tensor[pi, si, oi] = 1.0
    ...
    def get_all_intermediate_nodes(self, start_node: str, target_node: str) -> Set[str]:
        """Return all nodes on **any** path start→target (excluding endpoints),
           via two BFS passes (forward from start, backward from target)."""
        if start_node == target_node or start_node not in self.adjacency_list:
            return set()

        # 1) Forward BFS from start_node
        forward_seen = {start_node}
        queue = [start_node]
        while queue:
            u = queue.pop(0)
        ...
        # 2) Reverse BFS from target_node
        backward_seen = {target_node}
        queue = [target_node]
        while queue:
            u = queue.pop(0)
            for v in self.reverse_adjacency_list.get(u, ()):
                if v not in backward_seen:
                    backward_seen.add(v)
                    queue.append(v)

        # 3) Intersection minus the endpoints
        return (forward_seen & backward_seen) - {start_node, target_node}

    def check_if_nodes_are_connected(self, start_node: str, target_node: str, num_hops: int) -> bool:
        """Quick k-hop reachability check."""
        encountered = {start_node}
        for _ in range(num_hops):
            new = set().union(*(self.adjacency_list.get(n, set()) for n in encountered))
            encountered |= new
            if target_node in encountered:
                return True
        return False
...
class GeneSymptomClassifier:
    ...
    def __init__(self, graph_dir: str, device: torch.device):
        ...
        self.MAX_PATH_LENGTH = 4 # Original=4
    ...
    # Calculate score on a gene - symptom pair
    def compute_predicted_probability(
        self,
        gene: str,
        symptom: str,
        gene_node_weights_override: torch.Tensor | None = None
    ):
        # 1) look up indices
        g_idx = self.groundtruth.gene_indices_map[gene]
        if symptom not in self.graph.node_indices_map:
            return torch.tensor(0.0, device=self.device)
        h_idx = self.graph.node_indices_map[symptom]
    
        # 2) collect up to MAX_PATH_LENGTH-hop ego-subgraph around g_idx via BFS
        seen, front = {g_idx}, {g_idx}
        for _ in range(self.MAX_PATH_LENGTH):
            next_front = set()
            for u in front:
                for nbr in self.graph.adjacency_list[self.graph.nodes_list[u]]:
                    v_idx = self.graph.node_indices_map[nbr]
                    if v_idx not in seen:
                        seen.add(v_idx)
                        next_front.add(v_idx)
            front = next_front
        subnodes = list(seen)
        V_sub = len(subnodes)
    
        # If symptom not reached in 4 hops, its only baseline offset matters
        if h_idx not in seen:
            return torch.sigmoid(self.baseline_offset)
    
        # 3) build a single index‐mapping vector once
        mapping = torch.full((self.graph.num_nodes,), -1, dtype=torch.long, device=self.device)
        mapping[subnodes] = torch.arange(V_sub, device=self.device)
    
        # 4) assemble sparse_W_sub by gathering all predicate edges that stay inside subnodes
        all_idx = []
        all_vals = []
        for w, A_sp in zip(self.predicate_weights, self.graph.A_r_sparse):
            A_sp = A_sp.coalesce()
            row, col = A_sp.indices()
            val = A_sp.values() * w
            mr = mapping[row]
            mc = mapping[col]
            mask = (mr >= 0) & (mc >= 0)
            if mask.any():
                all_idx.append(torch.stack([mr[mask], mc[mask]], dim=0))
                all_vals.append(val[mask])
        # cat them into one sparse tensor
        idx = torch.cat(all_idx, dim=1)
        vals = torch.cat(all_vals)
        sparse_W_sub = torch.sparse_coo_tensor(idx, vals, (V_sub, V_sub), device=self.device).coalesce()
    
        # 5) one‐hot seed and gene‐specific weights
        v = torch.zeros(V_sub, device=self.device)
        v[mapping[g_idx]] = 1.0
        base_t = (gene_node_weights_override
                  if gene_node_weights_override is not None
                  else self.node_weights_tensor[self.groundtruth.gene_indices_map[gene]])
        t_g_sub = base_t[subnodes]
    
        # 6) streamed hops over the small subgraph
        score = 0.0
        sub_h = mapping[h_idx]
        for hop in range(1, self.MAX_PATH_LENGTH + 1):
            # propagate outwards (use transpose to keep same semantics)
            v = torch.sparse.mm(sparse_W_sub.transpose(0,1), v.unsqueeze(1)).squeeze(1)
            v = t_g_sub * v
            if hop >= 2:
                score += v[sub_h]
    
        # 7) add bias + sigmoid
        return torch.sigmoid(score + self.baseline_offset)
